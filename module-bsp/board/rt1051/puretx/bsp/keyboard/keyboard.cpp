...
namespace bsp::keyboard
{
    ...
    bool write(const drivers::I2CAddress &addr, const std::uint8_t *txBuff, const std::size_t size)
    {
        for (auto i = 0; i < maxTransmissionRetry; ++i) {
            auto ret = i2c->Write(addr, txBuff, size);
            if (ret == size) {
                return true;
            }
        }
        LOG_ERROR("Can't write to keyboard. I2C Error: %ld", i2c->GetLastError());
        return false;
    }
    ...
    {
        for (auto i = 0; i < maxTransmissionRetry; ++i) {
            auto ret = i2c->Read(addr, rxBuff, size);
            if (ret == size) {
                return true;
            }
        }
        LOG_ERROR("Can't read from keyboard. I2C Error: %ld", i2c->GetLastError());
        return false;
    }
